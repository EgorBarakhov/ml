...
def get_near_points(origin, points):
    near_points = []
    for point in points:
        if origin == point:
            continue
        if calculate_distance(point, origin) < CIRCLES_DISTANCE:
...
def find_all_near_green_points(point, green_points, founded_points=None):
    ...
    near_green_points = get_near_points(
        point, [item for item in green_points if item not in founded_points])
    if len(near_green_points) == 0:
        return []
    founded_points.extend(near_green_points)
    all_near_green_points.extend(near_green_points)
    for near_green_point in near_green_points:
        external_green_points = find_all_near_green_points(
            near_green_point, green_points, founded_points)
...
def random_points(points, cursor_point):
    ...
    for i in range(k):
        x = np.random.randint(
            cursor_point[0] - area_radius, cursor_point[0] + area_radius)
        y = np.random.randint(
            cursor_point[1] - area_radius, cursor_point[1] + area_radius)
        new_point = [x, y]
        should_add = True
            ...
            if event.type == pygame.QUIT:
                ...
                for i in range(len(new_points)):
                    ...
                    pygame.draw.circle(screen, color='black',
                                       center=new_points[i], radius=RADIUS)
                    pygame.display.update()
                ...
                for i, point in enumerate(points):
                    pygame.draw.circle(
                        screen, color=flags[i], center=point, radius=RADIUS)
                ...
                all_green_points = get_all_of_color_points(
                    points, flags, 'green')
                green_points_for_yellow = all_green_points.copy()
                all_yellow_points = get_all_of_color_points(
                    points, flags, 'yellow')
                ...
                while len(all_green_points) != 0:
                    ...
                    all_near_green_points = find_all_near_green_points(
                        init_green_point, all_green_points)
                    all_near_green_points.append(init_green_point)
                    all_green_points = [
                        item for item in all_green_points if item not in all_near_green_points]
                    cluster = {
                        "color": random_color(),
                    ...
                    for i, point in enumerate(all_near_green_points):
                        ...
                        pygame.draw.circle(screen, color=(
                            color[0], color[1], color[2]), center=point, radius=RADIUS)
                        pygame.display.update()
                        time.sleep(0.1)
                    clusters.append(cluster)

                for point in all_yellow_points:
                    cluster = get_closest_cluster(
                        point, clusters, green_points_for_yellow)
